    ...
    def testAddRemoveNodes(self):
        ...
        if (cli_command == "server-add"):
            expectedResults = {"services":['kv'], 'port':18091, 'hostname':self.servers[num + 1].ip,
                                   ...
                                   'user':self.main.rest_username, "real_userid:user":self.ldapUser, "ip":'::1', "remote:port":57457}
        ...
        if (cli_command in ["failover"]):
            ...
            for num in range(nodes_failover):
                ...
                options += " --force"
                output, error = remote_client.execute_couchbase_cli(cli_command=cli_command, options=options, cluster_host="localhost", user=self.ldapUser, password=self.ldapPass)
                expectedResults = {'source':source, "real_userid:user":self.ldapUser, 'user':self.main.rest_username, "ip":'::1', "port":57457, 'type':'hard', 'nodes':'[ns_1@' + self.servers[nodes_add - nodes_rem - num].ip + ']'}
                self.checkConfig(self.eventID, self.main, expectedResults)
        ...
        if (cli_command == "server-readd"):
            for num in range(nodes_readd):
                ...
                output, error = remote_client.execute_couchbase_cli(cli_command=cli_command, options=options, cluster_host="localhost", user=self.ldapUser, password=self.ldapPass)
                expectedResults = {'node':'ns_1@' + self.servers[nodes_add - nodes_rem - num ].ip, 'type':'full', "real_userid:user":self.ldapUser, 'source':source, 'user':self.main.rest_username, "ip":'::1', "port":57457}
                self.checkConfig(self.eventID, self.main, expectedResults)
