...
class x509tests(BaseTestCase):
    ...
    def setUp(self):
        ...
        if (self.dns is not None) or (self.uri is not None):
            x509main(self.main)._generate_cert(copy_servers, type=SSLtype, encryption=encryption_type, key_length=key_length, client_ip=self.ip_address, alt_names='non_default', dns=self.dns, uri=self.uri,wildcard_dns=self.wildcard_dns)
        else:
            x509main(self.main)._generate_cert(copy_servers, type=SSLtype, encryption=encryption_type, key_length=key_length, client_ip=self.ip_address,wildcard_dns=self.wildcard_dns)
