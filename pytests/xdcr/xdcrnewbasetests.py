...
class CouchbaseCluster:
    ...
    def create_default_bucket(
        ...
        if self.use_java_sdk:
            CollectionsRest(self.__main_node).async_create_scope_collection(
    ...
    def async_load_bucket(self, bucket, num_items, value_size=512, exp=0,
        ...
        task = self.__clusterop.async_load_gen_docs(self.__main_node, bucket.name, gen,
    ...
    def async_update_delete(
        ...
        if not self.use_java_sdk:
            ...
            for bucket in self.__buckets:
                num_items = CollectionsStats(self.__main_node).\
...
class XDCRNewBaseTest(unittest.TestCase):
    ...
    def verify_filtered_items(self, src_main, dest_main, replications, skip_index=False):
        ...
        src_count = self._get_doc_count(src_main)
        dest_count = self._get_doc_count(dest_main)
    ...
    def verify_mapping(self, src_main, dest_main):
        src_rest = CollectionsRest(src_main)
        dest_rest = CollectionsRest(dest_main)
        src_scopes = []
        dest_scopes = []
        #Assuming implicit mapping
        for bucket in RestConnection(src_main).get_buckets():
        ...
        for cb_cluster in self.__cb_clusters:
            for remote_cluster_ref in cb_cluster.get_remote_clusters():
                ...
                if not skip_mirroring_validation :
                    self.verify_mapping(src_cluster.get_main_node(), dest_cluster.get_main_node())
                if not skip_collection_key_validation:
                    self.verify_collection_doc_count(src_cluster.get_main_node(), dest_cluster.get_main_node())
