...
class QueryANSIMERGETests(QueryTests):
    ...
    def test_ansi_merge_delete(self):
        ...
        self.query = 'MERGE INTO %s as d USING %s as s on meta(d).id == meta(s).id when matched then ' \
                     'delete' % (self.query_bucket0, self.query_bucket1)
    ...
    def test_ansi_merge_delete_match(self):
        keys, _ = self._insert_gen_keys(self.num_items, prefix='merge_delete')
        self.query = 'MERGE INTO %s as d USING %s as s on meta(d).id == meta(s).id when matched then delete where' \
                     ' meta(s).id' ' = "%s"' % (self.query_bucket0, self.query_bucket1, keys[0])
        actual_result = self.run_cbq_query()
        self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
        self.assertEqual(actual_result['metrics']['mutationCount'], 1, 'Merge deleted more data than intended')
        self.query = 'select * from %s where meta().id = "%s"' % (self.query_bucket0, keys[0])
        self.run_cbq_query()
        self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
    ...
    def test_ansi_merge_delete_match_limit(self):
        ...
        self.query = 'MERGE INTO %s as d USING %s as s on meta(d).id == meta(s).id when matched then delete ' \
                     'limit 10' % (self.query_bucket0, self.query_bucket1)
        actual_result = self.run_cbq_query()
        self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
    ...
    def test_ansi_merge_delete_where_match(self):
        ...
        self.query = 'MERGE INTO %s as d USING %s s0 on meta(d).id == meta(s0).id when matched then delete where ' \
                     'meta(s0).id = "%s" AND s0.name = "%s"' % (self.query_bucket0, self.query_bucket1,
                                                                keys[0], "employee-1")
        actual_result = self.run_cbq_query()
        self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
        self.assertEqual(actual_result['metrics']['mutationCount'], 1, 'Merge deleted more data than intended')
    ...
    def test_ansi_merge_update_match_set(self):
        try:
            self.run_cbq_query(query='CREATE INDEX name on {0}(name)'.format(self.query_bucket0))

            self.query = 'INSERT into %s (key , value) VALUES ("%s", %s)' % (self.query_bucket1,
                                                                             "test", '{"name": "new1"}')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'INSERT into %s (key , value) VALUES ("%s", %s)' % (self.query_bucket0,
                                                                             "new1", '{"name": "new1"}')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            new_name = 'edit'
            self.query = 'MERGE INTO %s b1 USING %s b2 on b1.name == b2.name when matched then ' \
                         'update set b1.name="%s"' % (self.query_bucket0, self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT name FROM %s' % self.query_bucket0
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
    ...
    def test_ansi_merge_update_match_unset(self):
        try:
            ...
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'MERGE INTO %s b1 USING %s b2 on b1.name == b2.name when matched then ' \
                         'update unset b1.name' % (self.query_bucket0, self.query_bucket1)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT * FROM %s ' % self.query_bucket0
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
    ...
    def test_ansi_merge_update_match_set_hash(self):
        try:
            ...
            new_name = 'edit'
            self.query = 'EXPLAIN MERGE INTO %s b1 USING %s b2 use hash(probe) on b1.name == b2.name when matched ' \
                         'then update set b1.name="%s"' % (self.query_bucket0, self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING %s b2 use hash(probe) on b1.name == b2.name when matched ' \
                         'then update set b1.name="%s"' % (self.query_bucket0, self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT name FROM %s' % self.query_bucket0
            ...
            new_name = 'edit1'
            self.query = 'EXPLAIN MERGE INTO %s b1 USING %s b2 use hash(build) on b1.name == b2.name when matched ' \
                         'then update set b1.name="%s"' % (self.query_bucket0, self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING %s b2 use hash(build) on b1.name == b2.name when matched then ' \
                         'update set b1.name="%s"' % (self.query_bucket0, self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT name FROM %s where name == "edit1"' % self.query_bucket0
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.assertEqual(actual_result['results'][0]['name'], new_name, 'Name was not updated')
        finally:
    ...
    def test_ansi_merge_select_update_match_set_hash(self):
        try:
            ...
            # Check update merge with hash(probe)
            new_name = 'edit'
            self.query = 'EXPLAIN MERGE INTO %s b1 USING (select name from %s b2) as s use hash(probe) on' \
                         ' b1.name == s.name when matched then update set b1.name="%s"' % (self.query_bucket0,
                                                                                           self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING (select name from %s b2) as s use hash(probe) on b1.name == s.name' \
                         ' when matched then update set b1.name="%s"' % (self.query_bucket0,
                                                                         self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT name FROM %s' % self.query_bucket0
            ...
            # Check update merge with hash(build)
            new_name = 'edit1'
            self.query = 'EXPLAIN MERGE INTO %s b1 USING (select name from %s b2) as s use hash(build) on ' \
                         'b1.name == s.name when matched then update set b1.name="%s"' % (self.query_bucket0,
                                                                                          self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING (select name from %s b2) as s use hash(build) on ' \
                         'b1.name == s.name when matched then update set b1.name="%s"' % (self.query_bucket0,
                                                                                          self.query_bucket1, new_name)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
    ...
    def test_ansi_merge_json_update_match_set_hash(self):
        try:
            ...
            # Check update merge with hash(probe)
            new_name = 'edit'
            self.query = 'EXPLAIN MERGE INTO %s b1 USING ({"name":"new1"}) as s use hash(probe) on b1.name == s.name' \
                         ' when matched then update set b1.name="%s"' % (self.query_bucket0, new_name)
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING ({"name":"new1"}) as s use hash(probe) on b1.name == s.name when ' \
                         'matched then update set b1.name="%s"' % (self.query_bucket0, new_name)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT name FROM %s' % self.query_bucket0
            actual_result = self.run_cbq_query()
            ...
            # Check update merge with hash(build)
            new_name = 'edit1'
            self.query = 'EXPLAIN MERGE INTO %s b1 USING ({"name":"new2"}) as s use hash(build) on b1.name == s.name ' \
                         'when matched then update set b1.name="%s"' % (self.query_bucket0, new_name)
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING ({"name":"new2"}) as s use hash(build) on b1.name == s.name' \
                         ' when matched then update set b1.name="%s"' % (self.query_bucket0, new_name)
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT name FROM %s where name == "edit1"' % self.query_bucket0
    ...
    def test_ansi_merge_not_match_insert(self):
        try:
            ...
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT * FROM %s where meta().id == "test"' % self.query_bucket0
            actual_result = self.run_cbq_query()
    ...
    def test_ansi_merge_not_match_insert_use_index(self):
        try:
            self.run_cbq_query(query='CREATE INDEX name on {0}(name)'.format(self.query_bucket0))
            self.run_cbq_query(query='CREATE INDEX name2 on {0}(name)'.format(self.query_bucket0))
            ...
            self.query = 'EXPLAIN MERGE INTO %s b1 use index(name2) USING %s b2 on b1.%s == b2.%s when ' \
                         'not matched then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                                         '(KEY "test",VALUE {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['results'][0]['plan']['~children'][2]['~child']['~children'][0]['~child'][
                                 '~children'][0]['index'],
                             'name2', "The incorrect index is being used")

            self.query = 'MERGE INTO %s b1 use index(name2) USING %s b2 on b1.%s == b2.%s when not matched ' \
                         'then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                             '(KEY "test",VALUE {"name": "new"})')
            self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT * FROM %s where meta().id == "test"' % self.query_bucket0
            ...
            self.run_cbq_query(query="Drop index name ON {0}".format(self.query_bucket0))
            self.run_cbq_query(query="Drop index name2 ON {0}".format(self.query_bucket0))
    ...
    def test_ansi_merge_match_update_set_use_index_source(self):
        try:
            self.run_cbq_query(query='CREATE INDEX name on {0}(name)'.format(self.query_bucket0))
            self.run_cbq_query(query='CREATE INDEX name2 on {0}(name)'.format(self.query_bucket1))

            self.query = 'INSERT into %s (key , value) VALUES ("%s", %s)' % (self.query_bucket0, "test",
                                                                             '{"name": "new1"}')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')

            self.query = 'INSERT into %s (key , value) VALUES ("%s", %s)' % (self.query_bucket1, "test",
                                                                             '{"name": "new1"}')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            ...
            self.query = 'MERGE INTO %s b1 USING %s b2 use index(name2) on b1.%s == b2.%s when matched then ' \
                         'update set b1.%s = "ajay"' % (self.query_bucket0, self.query_bucket1, 'name', 'name', 'name')
            self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            ...
            self.run_cbq_query(query="Drop index name ON {0}".format(self.query_bucket0))
            self.run_cbq_query(query="Drop index name2 ON {0}".format(self.query_bucket1))
    ...
    def test_ansi_merge_select_not_match_insert(self):
        try:
            ...
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'MERGE INTO %s b1 USING (select * from %s b2) as s on b1.%s == s.%s when not matched' \
                         ' then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                              '(key "test", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
    ...
    def test_ansi_merge_json_not_match_insert(self):
        try:
            ...
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'MERGE INTO %s b1 USING ({"name":"new1"}) as s on b1.%s == s.%s when not matched then' \
                         ' insert %s' % (self.query_bucket0, 'name', 'name', '(key "test", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT * FROM %s where meta().id == "test"' % self.query_bucket0
            actual_result = self.run_cbq_query()
    ...
    def test_ansi_merge_not_match_insert_hash(self):
        try:
            ...
            self.query = 'EXPLAIN MERGE INTO %s b1 USING %s b2 use hash(probe) on b1.%s == b2.%s when not matched ' \
                         'then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                             '(KEY "test",VALUE {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING %s b2 use hash(probe) on b1.%s == b2.%s when not matched then' \
                         ' insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                         '(KEY "test",VALUE {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT * FROM %s where meta().id == "test"' % self.query_bucket0
            ...
            # Test JSON use hash
            self.query = 'EXPLAIN MERGE INTO %s b1 USING ({"name":"new1"}) as s use hash(probe) on b1.%s == s.%s ' \
                         'when not matched then insert %s' % (self.query_bucket0, 'name', 'name',
                                                              '(key "test1", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")
            self.query = 'MERGE INTO %s b1 USING ({"name":"new1"}) as s use hash(probe) on b1.%s == s.%s when not' \
                         ' matched then insert %s' % (self.query_bucket0, 'name', 'name',
                                                      '(key "test1", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT * FROM %s where meta().id == "test1"' % self.query_bucket0
            actual_result = self.run_cbq_query()
            self.assertEqual(len(actual_result['results']), 1, 'Query was not run successfully')

            # Test subquery use hash
            self.query = 'EXPLAIN MERGE INTO %s b1 USING (select * from %s b2) as s use hash(probe) on b1.%s == s.%s ' \
                         'when not matched then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                                              '(key "test2", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertTrue("HashJoin" in str(actual_result), "HashJoin is not being used")

            self.query = 'MERGE INTO %s b1 USING (select * from %s b2) as s use hash(probe) on b1.%s == s.%s' \
                         ' when not matched then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                                               '(key "test2", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT * FROM %s where meta().id == "test2"' % self.query_bucket0
    ...
    def test_ansi_merge_not_match_insert_hash_negative(self):
        try:
            ...
            # Test regular merge using hash
            self.query = 'EXPLAIN MERGE INTO %s b1 USING %s b2 use hash(build) on b1.%s == b2.%s when not ' \
                         'matched then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                                     '(KEY "test",VALUE {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertFalse("HashJoin" in str(actual_result), "HashJoin is being used")

            self.query = 'MERGE INTO %s b1 USING %s b2 use hash(build) on b1.%s == b2.%s when not matched then' \
                         ' insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                         '(KEY "test",VALUE {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            ...
            # Test JSON use hash
            self.query = 'EXPLAIN MERGE INTO %s b1 USING ({"name":"new1"}) as s use hash(build) on b1.%s == s.%s when' \
                         ' not matched then insert %s' % (self.query_bucket0, 'name', 'name',
                                                          '(key "test1", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertFalse("HashJoin" in str(actual_result), "HashJoin is being used")
            self.query = 'MERGE INTO %s b1 USING ({"name":"new1"}) as s use hash(build) on b1.%s == s.%s ' \
                         'when not matched then insert %s' % (self.query_bucket0, 'name', 'name',
                                                              '(key "test1", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            ...
            # Test subquery use hash
            self.query = 'EXPLAIN MERGE INTO %s b1 USING (select * from %s b2) as s use hash(build) on b1.%s == s.%s' \
                         ' when not matched then insert %s' % (self.query_bucket0, self.query_bucket1, 'name ',
                                                               'name', '(key "test2", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertFalse("HashJoin" in str(actual_result), "HashJoin is being used")

            self.query = 'MERGE INTO %s b1 USING (select * from %s b2) as s use hash(build) on b1.%s == s.%s when not' \
                         ' matched then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                                      '(key "test2", value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
    ...
    def test_mixed_merge_not_match_insert_negative(self):
        try:
            ...
            self.query = 'MERGE INTO %s b1 USING %s b2 on b1.%s == b2.%s when not matched then insert %s' \
                         % (self.query_bucket0, self.query_bucket1, 'name', 'name', '(VALUE {"name": "new"})')
            self.run_cbq_query()
    ...
    def test_mixed_merge_not_match_insert_negative_old(self):
        try:
            ...
            self.query = 'INSERT into %s (key , value) VALUES ("%s", %s)' \
                         % (self.query_bucket1, "test", '{"name": "new1"}')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')

            self.query = 'MERGE INTO %s b1 USING %s b2 on b1.%s == b2.%s when not matched then insert %s' \
                         % (self.query_bucket0, self.query_bucket1, 'name', 'name', '({"name": "new"})')
            self.run_cbq_query()
            self.assertFalse(True, "The query should have errored")
    ...
    def test_mixed_merge_not_match_insert_legacy(self):
        try:
            ...
            self.query = 'MERGE INTO %s b1 USING %s b2 on key b2.%s when not matched then insert %s' \
                         % (self.query_bucket0, self.query_bucket1, 'name', '(Key "test", Value {"name": "new"})')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT count(*) FROM %s' % self.query_bucket0
            ...
            self.assertTrue(
                "MERGE with ON KEY clause cannot have document key specification in INSERT action." in str(e))
    ...
    def test_mixed_merge_not_match_insert_legacy_index_hint(self):
        try:
            ...
            self.query = 'MERGE INTO %s b1 use index(name) USING %s b2 on key b2.%s when not matched then insert %s' \
                         % (self.query_bucket0, self.query_bucket1, 'name', '{"name": "new"}')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')
            self.query = 'SELECT count(*) FROM %s' % (self.query_bucket0
                                                      )
    ...
    def test_non_keyspace_json_target(self):
        try:
            keys, _ = self._insert_gen_keys(self.num_items, prefix='merge_delete')
            self.query = 'MERGE INTO ({"name":"new1"}) as s USING %s on meta(s).id == meta(%s).id when matched ' \
                         'then delete' % (self.query_bucket1, self.query_bucket1)
            self.run_cbq_query()
            self.assertFalse(True, "The query should have errored")
    ...
    def test_non_keyspace_select_target(self):
        try:
            keys, _ = self._insert_gen_keys(self.num_items, prefix='merge_delete')
            self.query = 'MERGE INTO (select * from %s b2) as s USING %s on meta(s).id == meta(%s).id when matched' \
                         ' then delete' % (self.query_bucket0, self.query_bucket1, self.query_bucket1)
            self.run_cbq_query()
            self.assertFalse(True, "The query should have errored")
        except CBQError as e:
            self.assertTrue("syntax error - at (" in str(e))
    ...
    def test_ansi_merge_json_update_match_set_index_hint(self):
        try:
            self.run_cbq_query(query='CREATE INDEX name on {0}(name)'.format(self.query_bucket0))
            self.query = 'INSERT into %s (key , value) VALUES ("%s", %s)' % (self.query_bucket0, "new1",
                                                                             '{"name": "new1"}')
            actual_result = self.run_cbq_query()
            self.assertEqual(actual_result['status'], 'success', 'Query was not run successfully')

            self.query = 'MERGE INTO %s b1 USING ({"name":"new1"}) as s use index(name) on b1.name == s.name when ' \
                         'matched then update set b1.name="%s"' % (self.query_bucket0, 'edit')
            self.run_cbq_query()
            self.assertFalse(True, "The query should have errored")
        except CBQError as e:
    ...
    def test_ansi_merge_select_update_match_set_index_hint(self):
        try:
            ...
            self.query = 'MERGE INTO %s b1 USING (select * from %s b2) as s use index(name) on b1.%s == s.%s when ' \
                         'not matched then insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                                         '(key "test2", value {"name": "new"})')
            self.run_cbq_query()
            self.assertFalse(True, "The query should have errored")
    ...
    def test_ansi_merge_hints_on_target_hash(self):
        try:
            ...
            # Test regular merge using hash
            self.query = 'MERGE INTO %s b1 use hash(probe) USING %s b2 on b1.%s == b2.%s when not matched then ' \
                         'insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                        '(KEY "test",VALUE {"name": "new"})')
            self.run_cbq_query()
            self.assertFalse(True, "The query should have errored")
        except CBQError as e:
            self.assertTrue("Keyspace reference cannot have join hint (USE HASH or USE NL)in MERGE"
                            " statement." in str(e))
        finally:
            self.run_cbq_query(query="DROP INDEX name ON {0}".format(self.query_bucket0))
    ...
    def test_ansi_merge_hints_on_target_keys(self):
        try:
            ...
            # Test regular merge using hash
            self.query = 'MERGE INTO %s b1 use keys("test") USING %s b2 on b1.%s == b2.%s when not matched then' \
                         ' insert %s' % (self.query_bucket0, self.query_bucket1, 'name', 'name',
                                         '(KEY "test",VALUE {"name": "new"})')
            self.run_cbq_query()
            self.assertFalse(True, "The query should have errored")
