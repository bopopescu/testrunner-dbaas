        ...
        for index_name_def in self._create_array_index_definitions():
            try:
                ...
                for aggr_func in aggregate_functions:
                    if self.aggr_distinct:
                        select_non_array_where_clause = "SELECT " + aggr_func + \
                                                        "(DISTINCT {0}) from %s USE INDEX (`%s`) where {1} GROUP BY {2}"
                        select_array_where_clause = "SELECT " + aggr_func + \
                                                    "(DISTINCT d.{0}) from %s d USE INDEX (`%s`) {1}  GROUP BY d.{2}"
                    else:
                        select_non_array_where_clause = "SELECT " + aggr_func + \
                                                        "({0}) from %s USE INDEX (`%s`) where {1} GROUP BY {2}"
                        select_array_where_clause = "SELECT " + aggr_func + \
                                                    "(d.{0}) from %s d USE INDEX (`%s`) {1}  GROUP BY d.{2}"
                            ...
                            for query_bucket in self.query_buckets:
                                for query_template in query_definitions:
                                    for index_name in index_name_def["index_names"]:
                                        query = query_template % (query_bucket, index_name)
                                        ...
                                        query_verification = self._verify_aggregate_query_results(result,
                                                                                                  query_template,
                                                                                                  query_bucket)
        ...
        for index_name_def in self._create_array_index_definitions():
            try:
                ...
                for aggr_func in aggregate_functions:
                    if self.aggr_distinct:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "(DISTINCT d.{0}) from %s d USE INDEX (`%s`)" \
                                                        "UNNEST d.`{1}` AS t where d.{2} GROUP BY t"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(DISTINCT d.{0}) from %s d USE INDEX (`%s`) {1}  GROUP BY t"
                    else:
                        select_non_array_where_clause = "SELECT " + aggr_func + \
                                                        "(d.{0}) from %s d USE INDEX (`%s`)" \
                                                        "UNNEST d.`{1}` AS t where d.{2} GROUP BY t"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(d.{0}) from %s d USE INDEX (`%s`) {1}  GROUP BY t"
            ...
            try:
                ...
                for aggr_func in aggregate_functions:
                    if self.aggr_distinct:
                        ...
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(DISTINCT d.{0}) from %s d USE INDEX (`%s`) {1} GROUP BY "
                    else:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "(d.{0}) from %s d USE INDEX (`%s`)" \
                                                        "UNNEST d.`{1}` AS t where d.{2} GROUP BY "
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(d.{0}) from %s d USE INDEX (`%s`) {1}  GROUP BY "
        ...
        for index_name_def in self._create_array_index_definitions():
            try:
                ...
                for aggr_func in aggregate_functions:
                    if self.aggr_distinct:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "(DISTINCT {0}) from %s USE INDEX (`%s`) where {1}"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(DISTINCT d.{0}) from %s d USE INDEX (`%s`) {1}"
                    else:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "({0}) from %s USE INDEX (`%s`) where {1}"
        ...
        for index_name_def in self._create_array_index_definitions():
            try:
                ...
                for aggr_func in aggregate_functions:
                    if self.aggr_distinct:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "(DISTINCT t) from %s d USE INDEX (`%s`)" \
                                                        "UNNEST d.`{0}` AS t where d.{1} GROUP BY d.{2}"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(DISTINCT t) from %s d USE INDEX (`%s`) {0}  GROUP BY d.{1}"
                    else:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "(t) from %s d USE INDEX (`%s`)" \
                                                        "UNNEST d.`{0}` AS t where d.{1} GROUP BY d.{2}"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(t) from %s d USE INDEX (`%s`) {0}  GROUP BY d.{1}"
                    for non_array_first_field in non_array_fields:
                        for non_array_second_field in non_array_fields:
                            if non_array_second_field == non_array_first_field:
                ...
                for aggr_func in aggregate_functions:
                    if self.aggr_distinct:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        ...
                                                        "UNNEST d.`{1}` AS t where d.{2} GROUP BY t"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(t) from %s d USE INDEX (`%s`) {1}  GROUP BY t"
                    else:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "(t) from %s d USE INDEX (`%s`)" \
                                                        "UNNEST d.`{1}` AS t where d.{2} GROUP BY t"
        ...
        for index_name_def in self._create_array_index_definitions():
            try:
                ...
                for aggr_func in aggregate_functions:
                    if self.aggr_distinct:
                        select_non_array_where_clause = "SELECT " + aggr_func +\
                                                        "(DISTINCT {0} * 2) from %s USE INDEX (`%s`)" \
                                                        " where {1} GROUP BY {2}"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(DISTINCT d.{0} * 2) from %s d USE INDEX (`%s`) {1} GROUP BY d.{2}"
                    else:
                        select_non_array_where_clause = "SELECT " + aggr_func + \
                                                        "({0} * 6) from %s USE INDEX (`%s`) where {1} GROUP BY {2}"
                        select_array_where_clause = "SELECT " + aggr_func +\
                                                    "(d.{0} * 7) from %s d USE INDEX (`%s`) {1}  GROUP BY d.{2}"
                    for non_array_first_field in non_array_fields:
                        for non_array_second_field in non_array_fields:
                            if non_array_second_field == non_array_first_field:
                                query_definitions = [select_non_array_where_clause.format(non_array_first_field["name"],
    ...
    def test_non_array_group_non_array(self):
        ...
        for index_name_def in self._create_array_index_definitions():
            try:
                ...
                non_array_fields = [index_name_def["fields"][1], index_name_def["fields"][2]]
                if self.aggr_distinct:
                    select_non_array_where_clause = "SELECT DISTINCT {0} from %s USE INDEX" \
                                                    " (`%s`) where {1} GROUP BY {2}"
    ...
    def test_array_groupby_expressions(self):
        ...
        for index_name_def in self._create_array_index_definitions():
            try:
                ...
                non_array_fields = [index_name_def["fields"][1], index_name_def["fields"][2]]
                if self.aggr_distinct:
                    select_non_array_where_clause = "SELECT (DISTINCT {0} * 2) from %s USE INDEX" \
                                                    " (`%s`) where {1} GROUP BY {2}"
                    select_array_where_clause = "SELECT (DISTINCT d.{0} * 2) from %s d USE INDEX" \
                                                " (`%s`) {1}  GROUP BY d.{2}"
                else:
        ...
        for arr_field in array_fields:
            ...
            for first_field in non_array_fields[:-1]:
                for second_field in non_array_fields[non_array_fields.index(first_field) + 1:]:
                    ...
                    temp_create_defns = [
                        create_index_clause.format(index_names[0], first_field["name"], second_field["name"],
                                                   arr_field_defn),
                        create_index_clause.format(index_names[1], first_field["name"], arr_field_defn,
                                                   second_field["name"]),
                        create_index_clause.format(index_names[2], arr_field_defn, first_field["name"],
                                                   second_field["name"])]
                    drop_index_clause = "DROP INDEX {0} ON %s"
                    ...
                    for query_bucket in self.query_buckets:
                        for definition in temp_create_defns:
                            index_names_defn["create_definitions"].append(definition % query_bucket)
                        for definition in temp_delete_defns:
                            index_names_defn["drop_definitions"].append(definition % query_bucket)
